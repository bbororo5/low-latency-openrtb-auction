...
RTB 환경에서는 하나의 광고 요청이 제한된 시간 안에 처리되어야 한다. 요청을 받은 시스템은 광고 슬롯, 지면, 디바이스, 최소 입찰가 등의 정보를 해석하고, 여러 입찰 참여자(`Bidder`)의 응답을 수집한 뒤 유효한 입찰 중 낙찰자와 낙찰가를 결정해야 한다.
...
이 프로젝트는 실제 SSP와 DSP 전체를 구현하지 않는다. 대신 게시자(Publisher), 광고 판매 측(SSP/Exchange), 광고 구매 측(DSP/Bidder), 광고주(Advertiser)의 관계를 단순화해 모델링한다.

기본 흐름은 `Publisher -> SSP/Exchange <-> DSP/Bidder <- Advertiser`로 본다. 본 프로젝트는 이 중 광고 판매 측의 경매 흐름과 광고 구매 측의 입찰 판단 흐름을 작게 모델링해, OpenRTB 요청/응답 기반 경매 핵심 경로를 검증한다.

포함하는 논리 역할:

- OpenRTB 요청 수신 및 검증
- 입찰 참여자(`Bidder`)의 입찰 여부 판단
- Bidder의 입찰가 결정
- 간단한 광고 타겟팅 및 후보 광고 캠페인 매칭
- 등록된 여러 Bidder로 BidRequest 전달
- BidResponse 생성
- 여러 BidResponse 수집
- 낙찰자와 낙찰가 결정
...
- 실제 매체사 연동
- 실제 외부 DSP 연동
- 어떤 Bidder에 요청을 보낼지 고르는 라우팅 최적화
- 광고 운영 백오피스
- 광고 노출/클릭/전환 측정
- 정산 및 과금
- 리포팅 시스템
...
- Bidder가 요청을 평가해 입찰 여부와 입찰가를 결정한다.
...
- 동시 요청 수와 Bidder 수 증가가 응답 시간(latency)에 미치는 영향을 측정한다.
...
광고 지면을 가진 쪽이다. 이 프로젝트에서는 실제 게시자 서비스를 구현하지 않고, Auction Client가 광고 요청을 발생시키는 역할로 대신한다.

`SSP / Exchange`

게시자의 광고 지면을 판매하는 쪽이다. BidRequest를 여러 Bidder에게 전달하고, 제한 시간 안에 도착한 BidResponse를 수집해 낙찰자와 낙찰가를 결정한다.

`DSP / Bidder`

광고주를 대신해 광고 지면을 구매하는 쪽이다. BidRequest를 평가해 입찰 여부와 입찰가를 결정하고 BidResponse 또는 no-bid를 반환한다.
...
BidRequest 전달, Bidder의 입찰 여부 판단, BidResponse 수집, 낙찰자/낙찰가 결정을 수행하는 시스템이다.

`Bidder`

BidRequest를 평가하고 입찰 응답 또는 no-bid를 반환하는 주체다. 이 프로젝트에서는 실제 외부 DSP가 아니라 시스템 내부 또는 테스트 환경에서 시뮬레이션 가능한 Bidder로 다룬다.
...
Bidder가 특정 광고 노출 기회에 대해 입찰 의사와 입찰가를 표현하는 응답이다.
...
Bidder 응답 수집과 낙찰자/낙찰가 결정을 완료해야 하는 제한 시간이다.
...
경매 규칙에 따라 결정된 낙찰가다. 초기 경매 규칙은 Tech Spec 또는 ADR에서 확정한다.
...
4. 시스템은 등록된 여러 Bidder에게 동일한 BidRequest를 전달한다.
5. 각 Bidder는 요청과 자신의 캠페인 데이터를 평가한다.
6. 각 Bidder는 입찰가가 담긴 BidResponse 또는 no-bid를 반환한다.
...
`FR-004: Bidder 입찰 여부 판단`

Bidder는 요청 정보를 기반으로 입찰 여부와 입찰가를 결정할 수 있어야 한다.

`FR-005: BidResponse 생성`

Bidder는 입찰 가능한 경우 지원 범위에 맞는 BidResponse를 생성해야 한다.

`FR-006: 여러 Bidder 요청 전달 및 응답 수집`

초기 범위에서 시스템은 등록된 모든 Mock Bidder에게 동일한 BidRequest를 전달하고, 여러 Bidder 응답을 수집할 수 있어야 한다.
...
시스템은 Bidder 결과를 정상 입찰 응답, 입찰하지 않음(no-bid), 응답 시간 초과(timeout), 잘못된 응답(invalid response), 늦게 도착한 입찰 응답(late bid)으로 구분해야 한다.
...
성능 검증은 문서화된 로컬 환경에서 수행한다. 목표는 보편적인 처리량 수치를 증명하는 것이 아니라, 같은 조건에서 동시 요청 수, Bidder 수, 응답 지연이 latency와 제한 시간 내 응답률에 어떤 영향을 주는지 측정하는 것이다.
...
성능 테스트는 Bidder 수 증가에 따른 응답 시간과 제한 시간 내 응답률 변화를 측정해야 한다.
...
- 초기 범위에서는 어떤 Bidder에 요청을 보낼지 고르는 라우팅 최적화를 다루지 않는다.
- 등록된 모든 Mock Bidder에게 동일한 BidRequest를 전달한다.
- 여러 Mock Bidder는 동일한 구현체를 서로 다른 설정과 캠페인 데이터로 실행해 모델링할 수 있다.
- 각 Mock Bidder는 지원 광고 타입, 응답 지연, 입찰가, 실패 모드가 다를 수 있다.
...
여러 Bidder 응답 수집, timeout 처리, 낙찰자와 낙찰가 결정을 구현한다.
...
- 초기 광고 타입은 배너만 다룰 것인가?
- Bidder는 내부 시뮬레이션으로 시작할 것인가, 별도 실행 단위로 둘 것인가?
- 여러 Bidder를 동일 구현체의 여러 설정/데이터 인스턴스로 둘 것인가, 별도 실행 단위로 둘 것인가?
- Bidder 라우팅 최적화는 후속 확장으로도 다룰 것인가?
