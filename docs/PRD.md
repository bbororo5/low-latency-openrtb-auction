...
이 프로젝트는 publisher/provider의 광고 슬롯 요청을 경량 SSP가 OpenRTB 2.6 입찰 요청(`BidRequest`)으로 정규화한 뒤, 경량 DSP의 입찰 응답(`BidResponse`)을 수집해 제한 시간 안에 낙찰자와 낙찰가를 결정하는 저지연 RTB 입찰 시스템을 정의한다.
...
RTB 환경에서는 하나의 광고 슬롯 요청이 제한된 시간 안에 처리되어야 한다. 요청을 받은 SSP는 provider와 placement 설정을 확인하고, 광고 슬롯, 광고 타입, 최소 입찰가 같은 경매 입력을 OpenRTB BidRequest로 구성한 뒤 여러 경량 DSP의 응답을 수집해 유효한 입찰 중 낙찰자와 낙찰가를 결정해야 한다.
...
> 대용량 광고 슬롯 요청 상황을 가정하고, SSP가 provider slot request를 검증해 OpenRTB BidRequest를 만든 뒤, 제한 시간 안에서 여러 입찰 응답(`BidResponse`) 수집을 수행하고, 응답 시간 초과(timeout), 잘못된 입찰 응답(invalid bid), 늦게 도착한 입찰 응답(late bid)을 제외해 유효한 입찰 응답 중 낙찰자와 낙찰가를 결정하는 것.
...
- provider slot request를 수신하고 필수 필드를 검증한다.
- SSP가 inventory 설정을 조회해 DSP에 전달할 BidRequest를 생성한다.
...
`Provider Slot Client`

광고 슬롯이 열렸다는 요청을 보내는 테스트 주체다. 실제 browser SDK, mobile SDK, ad tag를 구현하지 않고, 이 프로젝트에서는 provider-facing slot request를 재현하는 클라이언트로 간주한다.

`Publisher`

광고 지면을 가진 쪽이다. 이 프로젝트에서는 실제 게시자 서비스를 구현하지 않고, Provider Slot Client가 광고 슬롯 요청을 발생시키는 역할로 대신한다.

`경량 SSP`

게시자의 광고 지면을 판매하는 쪽을 작게 구현한 역할이다. provider slot request를 검증하고 inventory 설정을 조회해 OpenRTB BidRequest를 만든 뒤, 여러 경량 DSP에게 전달하고 제한 시간 안에 도착한 BidResponse를 수집해 낙찰자와 낙찰가를 결정한다. OpenRTB 문서에서는 이 역할을 SSP 또는 Exchange로 표현한다.
...
Provider Slot Client가 SSP에 보내는 프로젝트 입력이다. provider id, placement id, 광고 타입, 배너 크기 또는 동영상 조건, 응답 제한 시간 힌트를 포함한다. OpenRTB 표준 객체는 아니며, SSP가 BidRequest를 생성하기 위한 최소 입력이다.
...
1. Provider Slot Client가 경량 SSP에 provider slot request를 보낸다.
2. 경량 SSP는 provider id, placement id, 광고 타입, 슬롯 조건을 검증한다.
3. 경량 SSP는 inventory 설정을 조회해 bidfloor, 통화, 지원 광고 조건을 확정한다.
4. 경량 SSP는 DSP에 전달할 OpenRTB BidRequest를 생성한다.
5. 경량 SSP는 등록된 여러 경량 DSP에게 동일한 BidRequest를 전달한다.
6. 각 경량 DSP는 요청과 자신의 캠페인 데이터를 평가한다.
7. 각 경량 DSP는 입찰가가 담긴 BidResponse 또는 no-bid를 반환한다.
8. 시스템은 제한 시간 안에 도착한 응답만 수집한다.
9. 시스템은 잘못된 입찰 응답(invalid bid)과 늦게 도착한 입찰 응답(late bid)을 제외한다.
10. 시스템은 유효한 입찰 응답 중 낙찰자와 낙찰가를 결정한다.
11. 낙찰자가 있으면 낙찰 결과(winner result)를 반환한다.
12. 유효한 입찰 응답이 없으면 낙찰 없음 결과(no-winner result)를 반환한다.

## 8. Functional Requirements

`FR-001: Provider Slot Request 수신`

시스템은 provider id, placement id, 광고 타입, 배너 크기 또는 동영상 조건을 포함한 provider slot request를 받을 수 있어야 한다.

`FR-002: Provider Slot Request 검증`

시스템은 필수 필드가 누락되거나 처리할 수 없는 요청을 invalid request로 분류해야 한다.

`FR-003: Inventory 설정 조회와 BidRequest 생성`

경량 SSP는 provider slot request와 inventory 설정을 조합해 DSP에 전달할 OpenRTB BidRequest를 생성할 수 있어야 한다.

`FR-004: 광고 요청 정보 해석`

시스템은 광고 노출 기회, 광고 타입, 광고 크기, 최소 입찰가를 입찰 판단에 사용할 수 있어야 한다.

`FR-005: 경량 DSP 입찰 여부 판단`

경량 DSP는 요청 정보를 기반으로 입찰 여부와 입찰가를 결정할 수 있어야 한다.

`FR-006: BidResponse 생성`

경량 DSP는 입찰 가능한 경우 지원 범위에 맞는 BidResponse를 생성해야 한다.

`FR-007: 여러 경량 DSP 요청 전달 및 응답 수집`
...
- provider id 또는 placement id 누락
- 알 수 없는 placement
- placement와 요청 광고 타입이 맞지 않음
- 지원하지 않는 광고 타입
- 배너 크기 또는 동영상 조건이 placement 설정과 맞지 않음
...
- provider-facing 경로는 display banner와 simple video만 다룬다.
- 기존 `/openrtb/auction` 직접 입력 경로는 벤치마크와 호환성 검증용으로 유지한다.
