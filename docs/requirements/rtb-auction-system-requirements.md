...
- `dispatchAt`: SSP가 검증과 정규화를 마치고 DSP fan-out payload를 확정하는 시각이다.
- `outboundTmax`: `dispatchAt`에서 `bidCutoff`까지 남은 양의 정수 밀리초다. DSP에 보내는 OpenRTB `tmax`이며 `effectiveTmax`보다 클 수 없다.
...
### 3.3 Serving snapshot semantics

- inventory와 campaign의 원본 관리 및 동기화는 `OOS-004`다.
- 현재 release는 process 시작 시 준비된 값을 immutable serving snapshot으로 복사한다.
- 한 process lifetime 동안 snapshot은 바뀌지 않으므로 모든 요청은 동일한 snapshot version을 읽는다.
- snapshot 변경은 process 재시작으로만 반영한다. runtime reload, freshness SLA, rollback은 현재 계약이 아니다.
- hot path는 snapshot 원본이나 외부 저장소를 동기 호출하지 않는다.
...
| `FR-002` | P0 | SSP는 유효한 provider request와 inventory snapshot으로 immutable auction context와 OpenRTB `BidRequest` template 하나를 생성해야 한다. | context와 template이 같은 request, impression, media, floor, currency를 가리키고 context의 `effectiveTmax`가 `bidCutoff`를 결정한다. |
| `FR-003` | P0 | SSP는 한 경매에서 선택된 각 DSP에 의미상 동일한 `BidRequest`를 최대 한 번 전송해야 한다. | DSP별 request/imp ID와 payload 의미가 동일하고, 공통 `outboundTmax`가 남은 deadline budget을 넘지 않으며, 암묵적 retry가 없다. |
...
|---|---|---|
| `A-001` | inventory와 campaign 값은 process 시작 전에 준비되고 startup 시 immutable snapshot으로 고정된다. | runtime freshness/reload가 현재 release 범위에 들어올 때 |
