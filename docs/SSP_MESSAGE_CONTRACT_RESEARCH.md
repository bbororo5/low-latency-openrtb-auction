...
현재 SSP 메시지 계약의 기준 문서는 `API_SPEC.md`다. 이 문서는 업계 기준과 설계 근거를 보존하는 참고 자료이며, 구현 계약을 직접 바꾸지 않는다.
...
These notes explain where the current code still needs alignment with the message contracts promoted into `API_SPEC.md`.
...
Use this research note only as supporting context when reviewing or evolving the SSP message contracts in `API_SPEC.md`.

Do not add new implementation scope directly from this note. Promote an interface decision to `API_SPEC.md`, an implementation decision to `TECH_SPEC.md`, or an architectural decision to an ADR first. Then update code only where the current implementation violates the chosen contract.
