...
이 문서는 Product Requirements와 ASR을 바탕으로 시스템 경계, 주요 view, 런타임 구조를 설명한다.

품질속성과 측정 가능한 시나리오는 `architecture-significant-requirements.md`, API 세부 계약은 `api-interface-specification.md`, 내부 컴포넌트 상세 설계는 `implementation-technical-specification.md`, 데이터 상태 분류는 `data-architecture.md`에서 다룬다.

## 1. Introduction, Context & Scope

### 1.1 Purpose

이 문서는 Product Requirements에서 정의한 RTB 입찰 시스템을 어떤 구조로 바라볼지 정리한다.
...
제한 시간 내 응답, 낮은 tail latency, 낙찰 결과 일관성, 실패 격리, 관찰 가능성은 이 구조의 핵심 architecture drivers다. 상세 품질속성과 시나리오는 `architecture-significant-requirements.md`를 기준으로 한다.
...
이 장의 저장소 경계는 Data Architecture의 원칙을 따른다. 기준 데이터의 source of truth와 입찰 hot path에서 읽는 serving copy를 구분하고, 현재 아키텍처는 매 경매 요청마다 외부 저장소를 동기 조회하는 구조를 기본으로 두지 않는다.
...
경량 DSP는 입찰 판단에 필요한 캠페인 데이터를 내부 Campaign Repository 또는 인메모리 인덱스로 조회한다. 이 데이터 구조는 C2 컨테이너가 아니라 DSP 애플리케이션 내부 컴포넌트로 보고, Implementation Technical Spec의 C3/데이터 모델에서 다룬다.
...
측정 기준은 `architecture-significant-requirements.md`, 실제 metric 구성은 `observability.md`, 측정 결과는 `performance-baseline.md`와 `docs/performance-reports/*`에서 다룬다.
...
이 섹션은 현재 아키텍처 단계에서 확정하지 않고, Implementation Technical Specification 또는 구현/측정 단계에서 구체화할 항목을 정리한다.
