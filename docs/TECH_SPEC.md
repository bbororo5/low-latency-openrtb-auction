...
이 문서의 목적은 OpenRTB 기반 입찰 요청(`BidRequest`)이 들어왔을 때 시스템이 어떤 데이터를 읽고, 어떤 순서로 처리하며, 어떤 기준으로 입찰 응답(`BidResponse`)과 낙찰 결과를 만드는지 정의하는 것이다.
...
이 문서는 현재 시스템 범위에서 필요한 세부 설계를 다룬다.
...
## 7. Deferred Decisions & ADR Candidates
