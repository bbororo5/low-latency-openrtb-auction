...
C4Container
    ...
    Rel(dsp_application, regional_ledger, "리스 발급·페이싱·격리·정산", "입찰 경로 밖")
...
| 전역 책임 원장 | 총예산, 전역 예비액과 겹치지 않는 리전 책임 봉투 | PostgreSQL 스키마, 차단·복구 구현 |
| 리전 예산 원장 | 자기 리전 책임액, DSP 리스의 발급·페이싱·격리·반환 상태 | 저장 제품과 복구 사본 구현 |
| 지역 금액 사건 기록 | `lurl`·`burl`의 내부 최초·중복·만료 판정과 재생 가능한 캠페인 지출 근거 | PostgreSQL 스키마 |
...
지역 금액 사건 기록은 DSP가 실제로 접수한 통지와 내부 판정의 원본이다. 리전 예산 원장은 권한 책임 상태의 원본이며 사건마다 갱신하지 않는다. 리스 정산기가 모든 잠재 지출 기한이 끝난 리스의 확정 소비와 반환액을 집계하고, 지역 예산 원장은 같은 정산을 한 번만 반영한다. 정상 기한 안에 접수한 `burl`은 캠페인 확정 지출 근거가 되며 실제 청구·송금은 구현하지 않는다.

확정 지출은 리전 책임 봉투 안에서 리스 단위로 정산한다. 전역 책임 원장은 확정 지출을 실시간으로 모으지 않고 리전 간 겹치지 않는 책임 봉투와 예비액만 소유한다.
