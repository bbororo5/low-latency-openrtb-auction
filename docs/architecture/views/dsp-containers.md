...
C4Container
    ...
    Container_Boundary(project_dsp, "프로젝트 DSP") {
        ...
        ContainerDb(money_store, "금액 통지 저장소", "저장 기술 미정", "접수한 lurl·burl과 멱등 처리 상태를 보존한다.")
    }
...
| 금액 통지 저장소 | `lurl`·`burl`, 멱등 접수·처리 상태와 복구 가능한 캠페인 지출·SSP 지급 근거 | 저장 제품과 스키마 |
...
금액 통지 저장소는 DSP가 접수 성공을 반환한 사건의 원본이다. 리전 예산 원장은 권한 책임 상태의 원본이며 수신함의 사건을 비동기로 멱등 반영해 예약을 종결한다. 둘이 같은 저장 제품을 사용할지는 기술 선택에서 정하지만, 성공 접수를 원장 반영과 하나의 동기 트랜잭션으로 묶지 않는다. 정상 기한 안에 접수한 `burl`은 캠페인 확정 지출과 SSP 지급 근거가 되며, 실제 청구·송금은 구현하지 않는다.

`nurl`은 낙찰 사실만 알리며 예약 금액을 바꾸지 않는다. 따라서 금액 통지 저장소의 RPO 0 대상에 포함하지 않고 추적·분석용 경매 결과 자료로 취급한다.
