...
- 렌더링 증표는 `version`, `keyId`, `nonce`를 평문 헤더로 두고, `slotAuctionKey`, 낙찰 DSP·입찰·가격, 매크로를 치환한 `billingUrl`, SSP가 정한 2초 렌더링 기한을 최소 이진 페이로드로 직렬화한다. 데이터 압축은 사용하지 않는다. DSP의 `Bid.exp`와 `burl` 수락 상한은 증표 필드가 아니라 입찰 응답·통합 계약에서 해석한다.
...
- `slotAuctionKey`가 청구 고유 키다. 같은 유효 증표의 발급 리전 내 중복 청구는 저장소 고유 제약이 막고, 다른 SSP 리전은 그 청구를 대신 처리하지 않는다. DSP의 불투명 `burl` 처리는 같은 URL의 금액 효과를 한 번으로 수렴시킨다.
- 렌더링 확인·청구 처리는 `BillingClaimRecorded`의 내구 기록 성공까지만 소유한다. `burl` 전달과 재개는 DSP 경매·과금 통지가 소유한다.
- DSP 경매·과금 통지는 `nurl`·`lurl`을 단발 최선 노력으로 전달하고, `burl`만 `BillingClaimRecorded`를 원본으로 즉시 전달·내구 재시도한다. 프로젝트 DSP에는 `Imp.exp=2`, `Bid.exp=2`와 5초 `burl` 수락 상한을 통합 계약으로 적용하며, 재시도는 그 상한을 넘지 않는다.
