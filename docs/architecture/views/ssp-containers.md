...
C4Container
    ...
    Container_Boundary(ssp, "SSP") {
        ...
        ContainerDb(ssp_store, "SSP 지역 금액 사건 기록", "PostgreSQL", "청구 근거와 표준 통지 전달 결과를 추가 전용으로 보존한다.")
    }
...
| SSP 지역 금액 사건 기록 | `BillingClaimRecorded`와 SSP 자체 통지 전달 결과의 보존·재생 | 스키마와 비동기 병합 구현 |

경매와 렌더링·통지는 우선 같은 프로세스 안에서 실행 자원을 격리한다. 별도 프로세스는 현재 구조에 포함하지 않으며 통지 적체가 경매의 50ms 시간 예산을 실제로 침범할 때 재검토한다.

SSP는 경매 결과를 저장하지 않고 인증된 렌더링 증표로 반환한다. 유효한 증표가 돌아오면 지역 기록에 청구 근거를 추가하고 입찰 응답의 `burl`을 호출한다. DSP의 내부 사건명이나 응답 본문을 요구하지 않으며, 메시지 기반 시설·저장소·청구서·송금을 공유하지 않는다.
