...
C4Container
    ...
    Container_Boundary(ssp, "SSP") {
        ...
        ContainerDb(ssp_store, "SSP 사실 저장소", "저장 기술 미정", "낙찰과 과금 가능 렌더링·HTTP 통지 발신함을 보존한다.")
    }
    ...
    Rel(ssp_application, ssp_store, "낙찰·과금 가능 사실 기록과 발신함 처리")
...
|---|---|---|
| SSP 애플리케이션 | 요청 검증, DSP별 격리·병렬 호출, 1가격 낙찰, 렌더링 검증, 발신함 처리와 HTTP 통지 재전달 | 내부 실행 자원 격리 방식 |
| SSP 사실 저장소 | 공개한 낙찰, 과금 가능 렌더링, 수취 근거와 HTTP 통지 발신함 보존 | 저장 제품, 스키마, 분할·복제 구현 |

경매와 렌더링·통지는 우선 같은 프로세스 안에서 실행 자원을 격리한다. 별도 프로세스는 현재 구조에 포함하지 않으며 통지 적체가 경매의 50ms 시간 예산을 실제로 침범할 때 재검토한다.

SSP는 DSP 입찰 응답에 포함된 계약 주소를 HTTP로 호출한다. DSP와 메시지 기반 시설이나 저장소를 공유하지 않는다. 성공으로 접수한 과금 가능 렌더링과 `burl` 발신 책임은 SSP 수취 근거이며, 청구서·송금·매체사 지급은 이 컨테이너의 책임이 아니다.
