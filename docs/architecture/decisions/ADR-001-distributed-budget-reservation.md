...
미정산 DSP 리스 액면 안에서는 미사용 권한·잠재 지출·확정 지출 대기액·격리액으로 분류가 바뀐다. `burl`마다 지역 원장을 갱신하지 않고 리스 정산 때 확정 소비와 반환액을 한 번 반영한다. 이미 소비한 금액은 리전 책임 봉투 안에서 누적하며, 전역에는 미사용임을 증명한 금액만 반환한다.
...
3. 로컬 권한이 충분하면 원장 호출 없이 입찰을 반환한다. DSP가 죽어도 리전 원장은 해당 리스의 미정산 책임액을 즉시 회수하지 않는다.
...
7. `lurl`·`burl`은 표준 URL 호출로 받고, 금액 사건은 일반 HTTP 성공을 반환하기 전에 지역 장부에 멱등 기록한다.
8. `burl` 사건 하나마다 지역 예산 원장을 갱신하지 않는다.
9. 모든 잠재 지출의 통지 가능 기한이 끝나면 지역 사건을 리스별로 집계해 확정 소비와 미사용 반환액을 만든다.
10. 지역 예산 원장은 `leaseId`와 정산 세대로 같은 리스 정산을 한 번만 적용한다. 재리스 요청은 정산과 별개이며 기존 리스의 과금을 추론하는 근거로 쓰지 않는다.
...
| 금액 사건·리스 정산 | 원장과 권한은 유지하고 사건을 적체한 뒤 리스 단위로 재처리한다. | 지출 가시성과 책임액 반환 지연 |

리스 만료는 신규 예약 권한만 끝낸다. 이미 만든 예약은 `lurl`, `burl`, 예약 후 5초 만료 중 하나로 종결될 때까지 보호한다.
