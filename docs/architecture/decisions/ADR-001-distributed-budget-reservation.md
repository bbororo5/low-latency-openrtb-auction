...
이 ADR은 구조를 선택하며 다음 세부 사항은 별도 ADR로 결정한다.

1. [전역·리전 원장의 저장소, 분할, 복제와 쓰기 권한 차단 방식](ADR-002-multi-region-ledger-topology.md)
2. 리전·DSP 권한의 크기, 리스 기간, 보충 하한선과 파편화 상한
3. 예약 증표, 로컬 장애 복구, `lurl`·`burl`·95초 만료 상태 전이
4. 이벤트 로그, 실시간 집계, 배치 대조와 원장 반영 방식
5. 두 리전의 트래픽 진입·전환과 50ms 시간 예산
