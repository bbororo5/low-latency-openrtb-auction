...
flowchart TB
    subgraph PDSP["프로젝트 DSP 회사"]
        G["전역 책임 원장 · 논리 서비스<br/>캠페인 총예산·미할당 책임액<br/>분할·다중 리전 복제"]
...
굵은 실선은 강하게 보존하는 책임 이전, 점선은 입찰 경로 밖의 권한 보충이다. 입찰·예약 처리기는 로컬 권한에만 의존한다. 전역 책임 원장은 논리적 단일 권위이지 단일 인스턴스가 아니다.

전역 원장은 논리적으로 하나지만 단일 인스턴스나 단일 리전에 두지 않는다. 캠페인별로 분할하고 두 리전에 복제하여 하나의 물리 장애가 전체 캠페인에 영향을 주지 않게 한다. 구체적인 저장소와 복제·승격 방식은 후속 ADR에서 정한다.
