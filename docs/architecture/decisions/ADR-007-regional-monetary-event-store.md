...
- 성공 응답은 PostgreSQL 동기 커밋이 끝난 뒤 반환한다. 비동기 커밋은 사용하지 않는다.
...
flowchart TB
    ...
    SJ1 -. "비동기 집계" .-> SG["SSP 전역 대조 투영"]
    SJ2 -. "비동기 집계" .-> SG
    DJ1 -. "비동기 집계" .-> DG["DSP 전역 대조 투영"]
    DJ2 -. "비동기 집계" .-> DG
```
...
| 계약 | 구현 기준 |
|---|---|
| 최초 결과 고정 | DSP 결정 행의 `event_id` 고유 제약으로 `ACCEPTED`와 `EXPIRED_REJECTED` 중 하나만 삽입 |
| 결과 반복 반환 | 충돌 시 기존 결정과 요청 지문을 조회해 같으면 저장된 증거 반환, 다르면 거부 |
| SSP 사건 중복 방지 | `(event_id, event_type)` 고유 제약으로 청구·확정·미수집을 각각 한 번만 기록 |
| 내구 응답 | 트랜잭션 커밋 성공 뒤에만 HTTP 성공 또는 만료 거부 증거 반환 |
...
DSP의 기한 판정과 최초 결정 삽입은 하나의 트랜잭션에서 저장소 시각을 사용한다. 경쟁 요청은 고유 제약에서 직렬화되며, 패자는 커밋된 최초 결과를 읽어 반환한다.

## 3. 대안 비교

| 후보 | 적합한 점 | 현재 선택하지 않은 이유 |
|---|---|---|
| PostgreSQL | 고유 제약·트랜잭션·즉시 조회·재생·대조를 한 저장소에서 제공 | 연결 수, 파티션·인덱스와 장애 전환을 운영해야 함 |
| DynamoDB | 조건부 쓰기와 관리형 지역 고가용, 수평 확장 | 장기 순차 재생과 대조를 위해 Streams 밖의 추가 구조가 필요함 |
...
RDS Multi-AZ DB 인스턴스는 같은 리전의 다른 AZ에 동기 대기 복제본을 유지한다. 주 인스턴스 장애 중에는 해당 지역 금액 접수를 실패시키고 호출자가 같은 증표나 `eventId`로 재시도한다.

- 장애 전환 중에도 경매 Hot Path는 지역 금액 장부를 호출하지 않는다.
- 다른 리전은 자기 장부로 계속 처리한다.
- 중단된 리전 소유 사건은 다른 리전이 접수·거부를 대신 결정하지 않고 복구까지 재시도한다.
...
- 최초 과금 결과, 멱등 응답과 사건 원본을 한 트랜잭션 경계에서 설명할 수 있다.
- 별도 메시지 로그나 멱등 조회 저장소 없이 전달 재시도와 투영 재생을 구현한다.
- SSP·DSP 장부 대조를 SQL로 시작하고 규모가 커질 때 비동기 분석 저장소를 추가할 수 있다.
...
- 같은 `eventId`를 동시에 접수해도 결정 행은 하나이고 모든 응답 결과가 같다.
- 95초 경계 전후 요청이 경쟁해도 `ACCEPTED`와 `EXPIRED_REJECTED`가 함께 남지 않는다.
- 커밋 직후 애플리케이션을 종료해도 성공 응답한 사건을 재생할 수 있다.
- 작업자 종료·재시작과 중복 처리 뒤에도 전달 및 금액 투영 결과가 한 번이다.
