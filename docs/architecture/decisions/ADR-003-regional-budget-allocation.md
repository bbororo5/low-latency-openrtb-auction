...
여기서 비동기란 입찰이 이전 완료를 기다리지 않는다는 뜻이다. 책임액 차감과 활성화 자체는 강한 정합성으로 처리한다.

```mermaid
flowchart TB
    subgraph PDSP["프로젝트 DSP 회사 · 예산 제어 경로"]
        subgraph GLOBAL["전역 책임 원장 · 다중 리전 복제"]
        ...
        AUTH ==>|"강하게 보존한 책임액 활성화"| L1
        AUTH ==>|"강하게 보존한 책임액 활성화"| L2
    end
```
...
4. 전역 원장은 예비액을 원자적으로 격리하고 해당 리전의 책임액으로 이전한다.
5. 예비액이 부족하면 다른 리전이 미사용 책임액을 반환한 경우에만 다시 이전한다.
