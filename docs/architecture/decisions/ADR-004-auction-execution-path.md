...
- SSP는 낙찰 결과와 DSP 예약 증표를 인증된 렌더링 증표로 반환하고 경매 경로에서 낙찰 상태를 외부 저장소에 기록하지 않는다.
...
5. SSP는 낙찰·가격·DSP·예약 증표·렌더링 기한을 위조 방지 증표에 넣어 클라이언트에 반환한다. 렌더링 때 어느 SSP 인스턴스도 이 증표를 검증할 수 있어야 한다.
6. 공유 대기열, 공유 연결 고갈 또는 한 DSP의 지연이 다른 DSP의 실행 자원을 소비하지 못하게 한다.
7. 프로젝트 DSP의 로컬 자료가 준비되지 않았으면 기술 실패로 종료한다. 정상 자료는 있지만 적격 캠페인이나 로컬 예산 권한이 없으면 `NO_BID`하며, 어느 경우에도 외부 저장소를 기다리지 않는다.
...
- SSP 낙찰 결과를 상태 없는 증표로 넘겨 경매 성공 응답에 저장소 쓰기를 추가하지 않는다.
...
- 위조·만료·재사용된 렌더링 증표가 유효한 과금 사건을 만들지 않는다.
