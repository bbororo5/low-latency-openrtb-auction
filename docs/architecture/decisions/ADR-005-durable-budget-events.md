...
- DSP는 같은 `eventId`의 최초 결정을 `BillingAccepted` 또는 `BillingRejectedExpired`로 해당 리전 기록에 추가한다. 저장된 결정과 요청 핵심값이 일치할 때만 같은 결과를 반복 반환한다.
- SSP는 DSP의 접수 증거를 받으면 `BillingConfirmed`를 추가한다. DSP의 내구 만료 거부 증거를 받으면 `BillingUncollected`를 추가한다.
...
- 입찰을 처리하고 예약을 만든 DSP 리전이 금액 사건의 귀속 리전이다. `burl` 주소와 예약 증표가 이 리전을 식별하며 재시도도 같은 리전으로만 보낸다.
...
flowchart LR
    ...
    subgraph DSP["프로젝트 DSP 회사"]
        ...
        DJ["지역 추가 전용 기록<br/>BillingAccepted·RejectedExpired"]
        APPLY["예약·지출 투영"]
        DVIEW["지급·대조 투영"]
        API --> DJ
        DJ -.-> APPLY
...
| DSP | `BillingRejectedExpired` | 접수된 성공 기록 없이 기한이 지난 청구를 과금하지 않기로 확정함 |
| SSP | `BillingConfirmed` | DSP의 내구 접수 증거를 받음 |
| SSP | `BillingUncollected` | DSP의 내구 만료 거부 증거를 확인함 |
...
DSP는 저장소 시각으로 기한을 판정하고 `eventId`마다 두 결과 중 하나만 원자적으로 기록한다. `BillingAccepted`의 최초 내구 접수 시각이 95초 경계를 결정하며 투영이 늦어도 `BILLED`가 우선한다. 접수 이력이 없는 기한 뒤 요청은 `BillingRejectedExpired`로 고정한다. 만료 작업은 기한까지 접수된 지역 사건을 처리했다는 기준점을 확인하지 못하면 예약을 조기에 반환하지 않는다.

같은 `eventId`에 금액·캠페인·예약 식별자 등 핵심값이 다르게 들어오면 기존 결과를 재사용하지 않고 충돌로 거부한다. 최초 결정과 요청 지문을 함께 보존해야 멱등 키 재사용이 다른 금액에 영향을 주지 않는다.
...
| DSP 결정 기록 전 장애 | 결과 응답이 없으므로 SSP가 재전달 |
| DSP 결정 기록 뒤 응답 유실 | DSP가 기존 내구 접수·만료 거부 증거를 반복 반환 |
| DSP 귀속 리전 중단 | 다른 리전이 대신 판단하지 않고 `UNRESOLVED`로 유지한 뒤 귀속 리전에 재전달 |
...
- DSP가 접수하지 않은 기한 뒤 사건은 `BillingRejectedExpired`로 고정되고 캠페인 금액을 변경하지 않는다.
- 같은 `eventId`의 접수와 만료 거부를 경계 시각에 동시에 요청해도 하나의 내구 결정만 남는다.
- 같은 `eventId`로 핵심값을 바꾼 요청은 기존 결과를 재사용하지 않고 거부한다.
