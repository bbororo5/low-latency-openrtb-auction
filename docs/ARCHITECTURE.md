...
OpenRTB는 게시자(Publisher)의 광고 지면을 판매하는 쪽(SSP/Exchange)과 광고주(Advertiser)를 대신해 광고 지면을 구매하는 쪽(DSP/Bidder)이 실시간 입찰 요청과 응답을 주고받기 위한 표준 규약이다.

이 프로젝트는 OpenRTB 생태계 전체를 구현하지 않는다. 대신 입찰 요청이 들어오고, 입찰 여부와 입찰가를 판단하며, 유효한 입찰 응답 중 낙찰자와 낙찰가를 결정하는 핵심 흐름을 축소해서 다룬다.

이 프로젝트는 실제 SSP와 DSP 전체를 구현하지 않는다. 대신 광고 판매 측의 경매 흐름과 광고 구매 측의 입찰 판단 흐름을 작게 모델링해, OpenRTB 요청/응답 기반 경매 핵심 경로를 검증한다.

실제 시스템에서 판매 측은 광고 지면을 판매하고 경매를 수행하며, 구매 측은 광고주를 대신해 입찰 여부와 가격을 결정한다. 이 관계는 `Publisher -> SSP/Exchange <-> DSP/Bidder <- Advertiser`로 볼 수 있다.
...
`RTB Bidding System`은 OpenRTB 기반 입찰 요청을 받아 여러 Bidder로 요청을 전달하고, BidResponse를 수집해 낙찰자와 낙찰가를 결정하는 시스템이다. C1에서는 이 시스템을 Mock SSP/Exchange와 Mock DSP/Bidder의 핵심 경로를 함께 모델링하는 하나의 시스템 경계로 본다.

`Publisher / Auction Client`는 광고 요청이 발생하는 쪽이다. 실제 게시자 플랫폼을 구현하지 않고, 이 프로젝트에서는 테스트 가능한 요청 생성 주체로 간주한다.

`Advertiser / Campaign Setup`은 광고주 캠페인과 타겟팅 데이터를 준비하는 쪽이다. 실제 광고주 캠페인 관리 시스템은 구현하지 않고, 테스트 데이터 준비 역할로만 다룬다.

`Performance Test Runner`는 실제 트래픽을 대신해 부하 요청을 생성하는 외부 역할이다.
...
- 등록된 여러 Mock Bidder로 BidRequest 전달
- 입찰 판단에 필요한 요청 정보 해석
- 입찰 여부와 입찰가 판단
- BidResponse 생성 및 수집
...
어떤 Bidder에 요청을 보낼지 고르는 라우팅 최적화는 다루지 않는다. 등록된 모든 Mock Bidder에게 동일한 BidRequest를 전달하고, 각 Mock Bidder는 서로 다른 설정과 캠페인 데이터로 bid, no-bid, timeout, invalid bid, late bid를 시뮬레이션할 수 있다.
...
이 섹션은 OpenRTB BidRequest 한 건이 들어왔을 때, 시스템이 낙찰자/낙찰가 또는 낙찰 없음을 결정하기까지의 실행 흐름을 설명한다.
...
4. SSP/Exchange 역할은 동일한 BidRequest를 등록된 여러 Mock Bidder에게 전달한다.
5. 각 Mock Bidder는 사전에 준비된 데이터에서 요청 조건에 맞는 후보 광고 캠페인을 찾는다.
6. 각 Mock Bidder는 후보 광고 캠페인을 기준으로 입찰 여부와 입찰가를 판단하고 BidResponse 또는 no-bid를 반환한다.
7. SSP/Exchange 역할은 생성된 BidResponse를 수집하고, timeout, late bid, invalid bid를 낙찰 후보에서 제외한다.
8. 유효한 입찰 응답 중 경매 규칙에 맞는 응답을 낙찰자(winner)로 선택하고 낙찰가(auction price)를 결정한다.
9. 유효한 입찰 응답이 없으면 낙찰 없음(no-winner)을 정상 결과로 반환한다.
10. latency, timeout, invalid bid, no-winner 지표를 기록한다.

### 4.2 Performance-Critical Path

성능상 중요한 경로는 BidRequest 수신부터 낙찰자/낙찰가 또는 낙찰 없음 결정까지다.
...
입찰 판단에 필요한 데이터를 어디에서 가져오고 어떤 형태로 준비할지는 이 문서에서 확정하지 않는다. 원본 데이터를 매 요청마다 조회하지 않는다는 원칙만 두고, 구체적인 데이터 구조와 갱신 전략은 Tech Spec 또는 ADR에서 결정한다.
...
|---|---|---|
| 경매에 참여시킬 광고 요청 범위 | 아키텍처 단계에서는 광고 형식을 확정하지 않는다. 지원 광고 타입과 타입별 검증 규칙은 요청/응답 계약과 함께 Tech Spec에서 결정한다. | Tech Spec |
