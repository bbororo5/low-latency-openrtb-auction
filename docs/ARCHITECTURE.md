...
초기 범위에서는 어떤 Bidder에 요청을 보낼지 고르는 라우팅 최적화는 다루지 않는다. 등록된 모든 Mock Bidder에게 동일한 BidRequest를 전달하고, 각 Mock Bidder는 서로 다른 설정과 캠페인 데이터로 bid, no-bid, timeout, invalid bid, late bid를 시뮬레이션할 수 있다.
