...
#### 4.4.3 Timing And Competition Cases
...
#### 4.4.5 Duplicate And Retry Cases
...
성능 테스트 도구는 C1/C2 다이어그램의 별도 actor로 표현하지 않는다. 성능 테스트에서는 k6 같은 부하 테스트 도구가 `Publisher / Ad Slot` 역할을 대체해 provider slot request를 반복 전송한다고 본다. 기존 `/openrtb/auction` 직접 입력 경로는 비교용 benchmark path로 유지한다.
