...
RTB 경매의 시작점은 이미 완성된 OpenRTB BidRequest가 아니다. 광고 슬롯이 열렸다는 provider-facing 요청이 SSP에 들어오고, SSP가 이를 경매 가능한 DSP-facing BidRequest로 바꾼 뒤 DSP들에게 전달한다.

이 과정에서 여러 DSP가 동시에 응답할 수 있지만, 모든 DSP가 정상적으로 또는 제시간에 응답하지 않는다.
...
이 문제를 풀었다고 말하려면 최소한 다음을 검증할 수 있어야 한다.

| Criterion | Evidence |
|---|---|
| Request boundary | provider-facing 요청과 DSP-facing OpenRTB BidRequest가 코드와 테스트에서 구분된다. |
| Request normalization | 유효한 Provider Slot Request가 하나의 경매 실행 단위와 하나의 DSP-facing BidRequest로 변환된다. |
| Early rejection | 지원하지 않는 slot, media constraint, inventory는 DSP fan-out 전에 거절된다. |
| Concurrent DSP fan-out | SSP가 같은 BidRequest를 여러 경량 DSP에 전달하고 응답을 수집한다. |
| Deadline handling | deadline 이후 응답은 winner 후보에서 제외된다. |
| Failure classification | no-bid, timeout, late bid, invalid bid, no-winner가 구분된다. |
| Winner correctness | winner는 Bid Judgment를 통과한 valid bid 후보에서만 결정된다. |
| Observable result | latency, deadline 준수 여부, 결과 분류를 테스트나 메트릭으로 확인할 수 있다. |

처리량 수치만으로 성공을 주장하지 않는다. 이 프로젝트에서 먼저 증명해야 할 것은 제한 시간 안에서 올바른 결과를 반환하고 실패 유형을 구분하는 능력이다.
