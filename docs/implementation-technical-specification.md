# Implementation Technical Specification: OpenRTB 기반 저지연 RTB 입찰 시스템

이 문서는 Product Requirements와 Architecture에서 정의한 RTB 입찰 시스템을 실제 구현으로 옮기기 위한 세부 명세를 다룬다.

Product Requirements가 무엇을 해결할지 정의하고, Architecture가 어떤 구조로 바라볼지 정의한다면, Implementation Technical Spec은 개발자가 같은 기준으로 구현할 수 있도록 컴포넌트 책임, 패키지 경계, 처리 흐름, 실패 처리, 테스트 기준을 구체화한다.

데이터의 source of truth, derived serving state, transient input, event, observability 구분은 Data Architecture를 기준으로 한다. 이 문서는 그 기준을 구현 계약과 컴포넌트 책임으로 옮기는 역할을 한다.
...
- Product Requirements는 문제, 사용자 시나리오, 기능 요구사항, 성공 기준을 정의한다.
...
- Data Architecture는 source of truth, serving copy, 정합성, event/ledger 후보를 정의한다.
- Implementation Technical Spec은 API와 데이터 기준을 코드 컴포넌트, 패키지, 처리 규칙, 테스트 기준으로 옮긴다.
...
API 필드, endpoint, SSP-DSP OpenRTB subset, `AuctionResult` 상태, 내부 SSP 메시지 계약은 `api-interface-specification.md`를 기준으로 한다.

Implementation Technical Spec에서는 API 계약을 반복하지 않고, 그 계약을 구현 컴포넌트와 테스트 기준으로 어떻게 내리는지만 다룬다.

구현상 중요한 연결점:

| API contract | Implementation owner | Implementation Technical Specification section |
|---|---|---|
...
AuctionResult에는 `api-interface-specification.md`에서 정의한 필드를 포함한다. 특히 `elapsedMs`와 `dspResultCounts`는 기능 검증뿐 아니라 timeout, late bid, invalid bid 원인 분석에도 사용한다.
...
이 장은 현재 Implementation Technical Spec에서 확정하지 않는 결정을 정리한다. 여기의 항목은 기술명을 먼저 고르는 목록이 아니다. 각 항목은 요구사항, 제약, 측정 결과를 바탕으로 선택지를 좁혀가야 하는 ADR 후보이다.
