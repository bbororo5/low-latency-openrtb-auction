...
`/publisher/auction`은 제품형 기본 입력이다. `/openrtb/auction`은 이전 성능 측정과 OpenRTB 호환 경로 검증을 위한 보조 경로이며, provider-facing 기본 경로가 아니다.
...
`NO_WINNER`는 HTTP 500이 아니다. 경매는 정상적으로 끝났지만 낙찰 가능한 bid가 없었던 결과다.
...
| external DSP HTTP 204 no-bid | normal `NO_BID` |
